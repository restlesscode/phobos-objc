...
#import "PhobosObjcUIComponent.h"
...
static const CGFloat SVProgressHUDImageMargin = 28.0f;
static const CGFloat SVProgressHUDInnerMargin = 20.0f;
...
@property (nonatomic, strong) NSArray<UIImage*> *darkAnimationImages;
@property (nonatomic, strong) NSArray<UIImage*> *lightAnimationImages;
...
+ (SVProgressHUD*)sharedView {
    ...
    dispatch_once(&once, ^{
        sharedView = [[self alloc] initWithFrame: [SVProgressHUD getKeyWindowFrame]];
    });
#else
...
+(void)setAnimationImages:(NSArray<UIImage *> *)images {
    if (images && images.count != 0) {
        ...
        [self sharedView].imageView.animationImages = images;
        [[self sharedView].imageView startAnimating];
    }
}

+ (void)setDefaultLayoutStyle:(SVProgressHUDLayoutStyle)layoutStyle {
    [[self sharedView] setDefaultLayoutStyle:layoutStyle];
}
...
+(void)showAnimationLightImages:(NSArray<UIImage *> *)lightImages darkImages:(NSArray<UIImage *> *)darkImages imageDuration:(NSTimeInterval)imageDuration repeatTimes:(NSUInteger)repeatTimes status:(NSString *)status dismissDuration:(NSTimeInterval)dismissDuration {
    NSArray<UIImage*>* images = [NSArray new];
    if (@available(iOS 12.0, *)) {
        [self sharedView].lightAnimationImages = lightImages;
        [self sharedView].darkAnimationImages = darkImages;
        if ([self sharedView].traitCollection.userInterfaceStyle == UIUserInterfaceStyleDark) {
            images = darkImages;
        } else {
            images = lightImages;
        }
    ...
    [[self sharedView] showAnimationImageArray:images imageDuration:imageDuration repeatTimes:repeatTimes status:status dismissDuration:dismissDuration];
}
    ...
    if (self.imageView.animationImages) {
        if (@available(iOS 12, *)) {
            if (self.traitCollection.userInterfaceStyle == UIUserInterfaceStyleDark) {
                self.imageView.animationImages = self.darkAnimationImages;
            } else {
                self.imageView.animationImages = self.lightAnimationImages;
            }
        }else {
            self.imageView.animationImages = self.lightAnimationImages;
        }
...
-(void)showAnimationImageArray:(NSArray<UIImage *> *)imageArray imageDuration:(NSTimeInterval)imageDuration repeatTimes:(NSUInteger)repeat status:(NSString*)status dismissDuration:(NSTimeInterval)dismissDuration {
    ...
    [[NSOperationQueue mainQueue] addOperationWithBlock:^{
        ...
        if(strongSelf){
            ...
            // Update imageView
            strongSelf.imageView.animationImages = imageArray;
            strongSelf.imageView.animationDuration = imageDuration;
            strongSelf.imageView.animationRepeatCount = repeat;
            if (repeat == 1) {
                strongSelf.imageView.image = imageArray.lastObject;
            }
            strongSelf.imageView.hidden = NO;
            NSTimeInterval imagePerDuration = imageDuration/imageArray.count;
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(imagePerDuration * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            ...
            if (self.graceTimeInterval > 0.0 && self.backgroundView.alpha == 0.0f) {
                strongSelf.graceTimer = [NSTimer timerWithTimeInterval:self.graceTimeInterval target:strongSelf selector:@selector(fadeIn:) userInfo:@(dismissDuration) repeats:NO];
                [[NSRunLoop mainRunLoop] addTimer:strongSelf.graceTimer forMode:NSRunLoopCommonModes];
            } else {
                if (dismissDuration > 0) {
                    [strongSelf fadeIn:@(dismissDuration)];
                } else {
...
- (instancetype)initWithFrame:(CGRect)frame {
    if((self = [super initWithFrame:frame])) {
        ...
        // Set default values
        _defaultLayoutStyle = SVProgressHUDLayoutStyleDefault;
        ...
        _infoImage = [PhobosObjcUIComponent imageNamed:@"info"];
        _successImage = [PhobosObjcUIComponent imageNamed:@"success"];
        _errorImage = [PhobosObjcUIComponent imageNamed:@"error"];
...
- (void)updateHUDFrame {
    ...
    // condition of imageused imageview has image or imageview has animationimages
    BOOL imageUsed = ((self.imageView.image) && !(self.imageView.hidden)) || self.imageView.animationImages.count > 0;
    ...
    if(self.statusLabel.text) {
        ...
        if ((self.defaultLayoutStyle == SVProgressHUDLayoutStylePCN) && (self.minimumSize.width != 0))   {
            //按照设置的最小size，宽度固定，高度延伸计算label的高度
            constraintSize = CGSizeMake(self.minimumSize.width - SVProgressHUDLabelSpacing*2 , CGFLOAT_MAX);
        }
    ...
    //pcn style contentview height and width
    if ((self.defaultLayoutStyle == SVProgressHUDLayoutStylePCN) && self.minimumSize.width != 0) {
        hudWidth = self.minimumSize.width;
        if (self.imageViewSize.width == 0) {
            hudHeight = SVProgressHUDInnerMargin*2 + labelHeight;
        } else {
            hudHeight = SVProgressHUDImageMargin + SVProgressHUDInnerMargin*2 + contentHeight + labelHeight;
        }
    }
    ...
    // layout style of pcn
    if (self.defaultLayoutStyle == SVProgressHUDLayoutStylePCN) {
        self.imageView.center = CGPointMake(CGRectGetMidX(self.hudView.bounds), SVProgressHUDImageMargin + self.imageViewSize.height/2);
    }
    // Label
    if(imageUsed || progressUsed) {
        ...
        if (self.defaultLayoutStyle == SVProgressHUDLayoutStylePCN) {
            centerY = SVProgressHUDImageMargin + self.imageViewSize.height + SVProgressHUDInnerMargin + labelHeight/2;
        }
    } else {
        centerY = CGRectGetMidY(self.hudView.bounds);
    }
...
- (void)positionHUD:(NSNotification*)notification {
    ...
    self.frame = [SVProgressHUD getKeyWindowFrame];
...
- (void)moveToPoint:(CGPoint)newCenter rotateAngle:(CGFloat)angle {
    ...
    if (self.containerView) {
        ...
        if (self.defaultLayoutStyle == SVProgressHUDLayoutStylePCN) {
            CGFloat centerX = CGRectGetWidth([UIScreen mainScreen].bounds) / 2.0f;
            CGFloat halfHeightOfHud = CGRectGetHeight(self.hudView.bounds) / 2.0f;
            CGFloat centerY = (CGRectGetHeight([UIScreen mainScreen].bounds)-halfHeightOfHud*2) / 2.0f + halfHeightOfHud;
...
- (void)showImage:(UIImage*)image status:(NSString*)status duration:(NSTimeInterval)duration {
    self.darkAnimationImages = nil;
    self.lightAnimationImages = nil;
...
- (UIControl*)controlView {
    ...
    if (windowBounds.size.width == 0) {
        if (@available(iOS 13, *)) {
            ...
            if (windowScene != nil) {
                windowBounds = windowScene.windows.firstObject.frame;
            }
        }
    }
...
#pragma mark - get key window frame
...
+(CGRect)getKeyWindowFrame {
    ...
    if (frame.size.width == 0) {
        if (@available(iOS 13, *)) {
            UIWindowScene *windowScene = (UIWindowScene*)[[UIApplication sharedApplication] connectedScenes].allObjects.firstObject;
            if (windowScene != nil) {
                frame = windowScene.windows.firstObject.frame;
            }
        }
